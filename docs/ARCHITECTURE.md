...
## 🛡️ v10.0 企业化升级 (T5001–T5029)

v10.0 在 v9.x 产品形态之上叠加五大硬化支柱 (源自 `COVERAGE-AUDIT-v10.md` 五份审查):

| 支柱 | 任务 | 落地点 |
|---|---|---|
| **强契约 Agent Gateway** | T5001–T5003 | `agents/gateway.py` 统一入口 + `agents/contracts.py` Pydantic 契约 + PII/注入守卫 + `agents/prompts.py` 外置模板 + `services/platform/errors.py`/`retry.py` 错误信封与熔断 |
| **数据库级多租户** | T5010–T5013 | `tenant_id` 全覆盖 + RLS `USING`/`WITH CHECK` 双策略 + 复合/INCLUDE 索引 + 触发器去重 + pgvector 调优 + 分区/全文/备份 |
| **安全 fail-fast + PII/KMS** | T5014–T5015 | 启动 fail-fast + PII AES-GCM 字段级加密 + KMS/Doppler 密钥 |
| **三域隐私合规** | T5016 | GDPR Art.15/17/20/30/33 + PIPL 跨境声明 + CCPA/CPRA Do-Not-Sell/Share + GPC (见 [COMPLIANCE.md](./COMPLIANCE.md)) |
| **真实模型 AI 子系统** | T5020–T5028 | RAG 流式 + Mem0 真实抽取 + Multi-Agent + Plugin 容器签名 + WorkflowEngine 持久化 + EventBus Redis Streams/DLQ + Prompt v2 eval/热重载/成本追踪 |

### Agent Gateway 数据流

```
Caller → AgentGateway.invoke(agent, payload)
          ├─ Pydantic 输入契约 (agents/contracts.py)
          ├─ PII 政策 + Prompt-injection 守卫
          ├─ 外置 Prompt 模板 (agents/prompts.py)
          └─ Agent → Pydantic 输出契约 → 标准错误信封 (services/platform/errors.py)
```

### 边界硬化 (T5017–T5019)

- 三级 Rate Limit (全局/租户/用户) + Webhook SSRF 防护 + CSRF 双提交 Cookie
- LLM Prompt Injection 防护 (输入消毒 + out-of-band 校验) + SSO/JIT 治理
- 容器/CI 加固 (最小权限镜像 + 依赖锁定 + detect-secrets) + SOC2 证据矩阵 ([SOC2_EVIDENCE.md](./SOC2_EVIDENCE.md), 56 控制)

> v10.0 发布说明见 [releases/v10.0.0.md](./releases/v10.0.0.md)。
