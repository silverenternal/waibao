# Agent A — Task 15: Admin Endpoints + Monitoring

## Mission
Build all admin-only API endpoints: platform statistics, adapter health status, AI pipeline monitoring, deduplication review queue, and user management CRUD. All endpoints enforce admin role checks.

## Context
This is Day 5. The admin persona needs full platform visibility — how many candidates are in the system, which adapters are healthy, what is the AI extraction queue status, which dedup candidates need review, and managing user accounts. These endpoints power the admin dashboard that Agent B builds.

## Prerequisites
- Task 12 complete (signal tracking + analytics — funnel data, trending skills)
- Task 06 complete (deduplication pipeline — pending merges in review queue)
- Task 04 complete (adapters — adapter registry for health checks)
- Task 07 complete (AI extraction pipeline — queue and processing stats)
- Task 03 complete (FastAPI skeleton with auth + role checks)

## Checklist
- [ ] Create `backend/api/admin.py` — all admin endpoints
- [ ] Implement platform stats endpoint (totals, growth)
- [ ] Implement adapter health endpoint (last sync, errors, records)
- [ ] Implement AI pipeline monitoring endpoint (queue, stats, confidence)
- [ ] Implement dedup review queue endpoints (list pending, approve/reject)
- [ ] Implement user management CRUD
- [ ] Register router in `backend/main.py`
- [ ] Create `backend/tests/test_admin.py` — unit tests
- [ ] Run tests, verify pass
- [ ] Commit: "Agent A Task 15: Admin endpoints + monitoring"

## Implementation Details

### Admin Endpoints (`backend/api/admin.py`)
...
from backend.contracts.shared import UserRole
...
async def trigger_adapter_sync(
    ...
    from backend.adapters.base import get_adapter
...
async def get_pipeline_status(
    ...
    low_confidence = supabase.table("candidates").select("id", count="exact") \
        .lt("extraction_confidence", 0.7) \
        .not_.is_("extraction_confidence", "null").execute()
    ...
    # Embedding coverage
    with_embedding = supabase.table("candidates").select("id", count="exact") \
...
async def get_dedup_queue(
    status: str = Query(default="pending", regex="^(pending|approved|rejected)$"),
    ...
    """
    query = supabase.table("dedup_queue").select("*") \
        .eq("status", status) \
    ...
    for item in (result.data or []):
        candidate_a = supabase.table("candidates") \
            .select("id, first_name, last_name, email, location, skills") \
            .eq("id", item["candidate_a_id"]).single().execute()
        candidate_b = supabase.table("candidates") \
            .select("id, first_name, last_name, email, location, skills") \
            .eq("id", item["candidate_b_id"]).single().execute()
...
async def resolve_dedup(
    ...
    """
    # Load queue item
    item = supabase.table("dedup_queue").select("*") \
        .eq("id", str(queue_item_id)).single().execute()
    if not item.data:
        raise HTTPException(status_code=404, detail="Queue item not found")

    if decision.action == "merge":
        # Perform merge: keep primary, merge secondary's unique data
        primary_id = str(decision.primary_id) if decision.primary_id else item.data["candidate_a_id"]
        secondary_id = item.data["candidate_b_id"] if primary_id == item.data["candidate_a_id"] else item.data["candidate_a_id"]

        # Load both candidates
        primary = supabase.table("candidates").select("*") \
            .eq("id", primary_id).single().execute()
        secondary = supabase.table("candidates").select("*") \
            .eq("id", secondary_id).single().execute()
        ...
        if primary.data and secondary.data:
            ...
            merged_sources = (primary.data.get("sources") or []) + (secondary.data.get("sources") or [])
...
async def get_user(
    ...
    signals = supabase.table("signals").select("event_type", count="exact") \
        .eq("actor_id", str(user_id)).execute()

    result.data["total_actions"] = signals.count or 0
...
async def update_user(
    ...
    result = supabase.table("users").update(update) \
...
async def deactivate_user(
    ...
    result = supabase.table("users").update({"is_active": False}) \
        .eq("id", str(user_id)).execute()
    if not result.data:
        raise HTTPException(status_code=404, detail="User not found")
...
### Migration for Dedup Queue and Users Table

Add to migration or create `supabase/migrations/005_admin_tables.sql`:
...
CREATE INDEX idx_dedup_queue_status ON dedup_queue(status);
CREATE INDEX idx_dedup_queue_confidence ON dedup_queue(confidence DESC);

-- Users table (if not already created in auth migration)
...
from backend.api.admin import _merge_skills, _merge_experience, UserCreate, UserUpdate
from backend.contracts.shared import UserRole
...
- `supabase/migrations/005_admin_tables.sql`

## Acceptance Criteria
1. `GET /api/admin/stats` returns platform totals, active counts, and 7-day growth — admin only
2. `GET /api/admin/adapters/health` returns health for all three adapters — admin only
3. `POST /api/admin/adapters/{name}/sync` triggers re-sync — admin only
4. `GET /api/admin/pipeline/status` returns extraction queue, confidence distribution, embedding coverage — admin only
5. `GET /api/admin/dedup/queue` returns pending dedup items with enriched candidate data — admin only
6. `POST /api/admin/dedup/{id}/resolve` merges or separates candidates, updates queue — admin only
7. Merge logic correctly deduplicates skills (keeps higher years) and experience (dedup by company+title)
8. `GET /api/admin/users` lists users with role and active filters — admin only
9. `POST /api/admin/users` creates a user — admin only
10. `PATCH /api/admin/users/{id}` updates user fields — admin only
11. `DELETE /api/admin/users/{id}` soft-deletes (deactivates) — admin only
12. Every endpoint returns 403 for non-admin users
13. All tests pass: `python -m pytest tests/test_admin.py -v`

## Handoff Notes
- **To Task 16:** Seed data should populate the dedup queue with 5-10 pending review items. Create users for all demo personas. Set adapter health to realistic states.
- **To Agent B:** Admin endpoints are all under `/api/admin/`. Platform stats return `totals`, `active`, and `growth_7d` objects. Pipeline status has `extraction_queue`, `confidence_distribution`, and `embedding_coverage`. Dedup queue items include both candidate summaries side-by-side. User management is standard CRUD. All endpoints require admin role — show appropriate access-denied UI for non-admins.
- **Decision:** Dedup merge is a simple field-level merge (keep higher years, combine sources). User deletion is soft (is_active=false) to preserve signal history and audit trail. Adapter health is in-memory (reset on server restart) — this is a PoC pattern, not production.
