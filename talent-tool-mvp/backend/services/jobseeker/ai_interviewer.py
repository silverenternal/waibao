...
from services.jobseeker.question_bank import Question, question_bank
...
# T1801 - 真实 GPT-4V 配置
# ---------------------------------------------------------------------------
# 默认 evaluation model:生产用 gpt-4o (GPT-4V) — 综合视频理解 + JSON 质量稳定
# mock/成本测试用 gpt-4o-mini
# 真实调用走 providers.llm.openai_provider;若 OPENAI_API_KEY 不存在则 fallback mock
REAL_MODEL_EVAL = "gpt-4o"
REAL_MODEL_SUMMARY = "gpt-4o"
FALLBACK_MODEL = "gpt-4o-mini"

VISION_PROMPT_TEMPLATE_ZH = (
    "你是 AI 结构化面试的视频分析助手。基于提供的候选人面试视频截图/描述,"
    "请捕捉以下信号,严格 JSON 输出:\n"
    "{\n"
    ' "confidence": 0-100,  // 整体表达自信度(眼神/姿态/语速)\n'
    ' "nervous_signals": 0-100,  // 紧张迹象(回避眼神/重复/停顿)\n'
    ' "engagement": 0-100,  // 投入度(身体前倾/手势/语调起伏)\n'
    ' "environment": "...",  // 简短环境描述 (如 office / quiet home / noisy)\n'
    ' "notes": "中文 1-2 句具体观察"\n'
    "}\n"
    "不要打分,只描述事实。"
)
...
class AIInterviewer:
        ...
        *,
        ...
        try:
            ...
            resp = await self.llm.chat(
                ...
                model=os.environ.get("AI_INTERVIEW_SUMMARY_MODEL", REAL_MODEL_SUMMARY),
                temperature=0.4,
                max_tokens=900,
    ...
    async def _llm_score(
        ...
        try:
            resp = await self.llm.chat(
                messages=[Message(role="user", content=prompt)],
                model=os.environ.get("AI_INTERVIEW_EVAL_MODEL", REAL_MODEL_EVAL),
                temperature=0.3,
                max_tokens=700,
                response_format={"type": "json_object"},
            )
    ...
    async def _safe_vision(self, *, video_url: str | None, question: Question) -> str | None:
        """T1801 - 真实 GPT-4V 视频理解(若 video_url 非空)。

        优先级:
            1. vision.describe_video / analyze_video(若 provider 支持 30s 视频)
            2. vision.describe_frame(用首帧) + 增量帧
            3. 截屏 URL(模拟 fallback)
        """
        if not video_url:
            return None
        try:
            # 真实 GPT-4V 接口:vision.analyze_video / describe_frame / analyze
            describe = (
                getattr(self.vision, "describe_video", None)
                or getattr(self.vision, "analyze_video", None)
                or getattr(self.vision, "describe_frame", None)
                or getattr(self.vision, "analyze", None)
            )
            if describe is None:
                return None
            prompt = VISION_PROMPT_TEMPLATE_ZH + f"\n相关题目:{question.title}"
            try:
                out = await describe(video_url, prompt=prompt)
            except TypeError:
                # provider 的 describe 签名不接 prompt 参数
                out = await describe(video_url)
            if isinstance(out, dict):
                parts = []
                if out.get("confidence") is not None:
                    parts.append(f"置信 {out['confidence']}")
                if out.get("nervous_signals") is not None:
                    parts.append(f"紧张 {out['nervous_signals']}")
                if out.get("engagement") is not None:
                    parts.append(f"投入 {out['engagement']}")
                if out.get("environment"):
                    parts.append(f"环境 {out['environment']}")
                if out.get("notes"):
                    parts.append(out["notes"])
                summary = ", ".join(parts)[:400] if parts else json.dumps(out, ensure_ascii=False)[:400]
                return summary
            return str(out)[:400]
        except Exception as e:  # noqa: BLE001
