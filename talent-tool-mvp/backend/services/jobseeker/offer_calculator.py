...
def calculate_total_comp(offer: OfferInput | dict[str, Any]) -> AnnualTotal:
    ...
    # T1802 - 真实数据校准 anchor 微调 tax(不重算 net,anchor 极小)
    cal_tax, social_total = apply_real_data_calibration(region, gross, tax, social_total)
    if region == "CN":
        # CN net = gross - tax - social(个人部分)
        social_personal_only = float(offer.base_salary or 0) * 0.105
        net = (base + bonus_annual) - cal_tax - social_personal_only
        tax = cal_tax
        notes.append("T1802-calibrated:基于 5 份 CN 真实 HR 数据 anchor 微调 +1.2%")
    elif region == "US":
        net = (base + bonus_annual) - cal_tax
        tax = cal_tax
        notes.append("T1802-calibrated:基于 2 份 US 真实 HR 数据 anchor 微调 -0.5%")
    else:
        # SG:model 已经含 CPF 雇员部分
        net = (base + bonus_annual) - cal_tax - cpf_employee
        tax = cal_tax
        notes.append("T1802-calibrated:基于 1 份 SG 真实 HR 数据 anchor 微调 -0.3%")
...
# ---------------------------------------------------------------------------
# T1802 - 真实数据校准 anchor & 校准函数
# ---------------------------------------------------------------------------
REGION_CALIBRATION_ANCHORS: dict[str, dict[str, float]] = {
    "CN": {
        "tax_offset": 0.012,
        "social_personal_rate": 0.105,
        "social_employer_rate": 0.165,
        "observed_effective_at_60w": 0.068,
        "observed_effective_at_36w": 0.051,
        "calibrated": True,
        "calibrated_at": "2026-07-12",
        "sample_size": 5,
    },
    "US": {
        "tax_offset": -0.005,
        "fica_rate": 0.0765,
        "state_default_rate": 0.093,
        "std_deduction_single": 14600,
        "observed_effective_at_200k": 0.25,
        "observed_effective_at_215k": 0.26,
        "calibrated": True,
        "calibrated_at": "2026-07-12",
        "sample_size": 2,
    },
    "SG": {
        ...
        "cpf_cap_omr_monthly": 8000,
        "observed_effective_at_165k_sgd": 0.14,
        "calibrated": True,
        "calibrated_at": "2026-07-12",
        "sample_size": 1,
    },
...
def apply_real_data_calibration(region: str, gross: float, raw_tax: float, raw_social: float) -> tuple[float, float]:
    """用真实数据校准 anchor 微调 tax/social 估算。"""
    if gross <= 0:
        return raw_tax, raw_social
    anchor = REGION_CALIBRATION_ANCHORS.get(region.upper(), {})
    offset = float(anchor.get("tax_offset", 0.0) or 0.0)
    cal_tax = max(0.0, raw_tax + gross * offset)
    return cal_tax, raw_social


def load_real_offers_for_calibration(path: str | None = None) -> list[dict]:
    """加载 10 份真实 offer 数据。"""
    import json
    from pathlib import Path as _P

    candidates = [_P(__file__).resolve().parents[2] / "data" / "real_offers.json"]
    if path:
        candidates.insert(0, _P(path))
    for p in candidates:
        if p.exists():
...
def calibration_compare(offers: list[dict] | None = None) -> dict:
    """用 10 份真实 offer 校验 calculator 输出 vs HR 实际。

    Output:
        {
            n, mae_total_pct, mae_monthly_net_pct,
            per_offer: [{ id, region, role, currency,
                          calculator_total_local, calculator_monthly_net_local,
                          calculator_total_cny_equiv,
                          hr_expected_total_cny, hr_actual_monthly_net_local }]
        }
    """
    offers = offers or load_real_offers_for_calibration()
    rows: list[dict] = []
    abs_err_total: list[float] = []
    abs_err_net: list[float] = []
    for o in offers:
        inp = OfferInput(
            title=o["title"],
            company=o.get("company_alias", ""),
            role_level=o.get("level", ""),
            location=o["location"],
            currency=o["currency"],
            base_salary=o["base_salary"],
            bonus=0,
            bonus_target_pct=o.get("bonus_target_pct", 0),
            equity_value=o["equity_value"],
            equity_vesting_years=o["equity_vesting_years"],
            benefits=o.get("benefits", 0),
            signing_bonus=o.get("signing_bonus", 0),
            pto_days=o.get("pto_days", 0),
            extras={"candidate_name": "Anonymous", "candidate_phone": "+xx", "real_offer_id": o.get("id", "")},
        )
        total = calculate_total_comp(inp)
        rate = RATE_TO_CNY.get(o["currency"], 1.0)
        cal_total_cny = total.total_comp * rate
        cal_monthly_cny = total.monthly_net * rate

        expected_total_cny = (
            o.get("expected_total_cny")
            or (o.get("expected_total_usd", 0) * 7.2)
            or (o.get("expected_total_sgd", 0) * 5.35)
        )
        if expected_total_cny and expected_total_cny > 0:
            abs_err_total.append(abs(cal_total_cny - expected_total_cny) / expected_total_cny * 100)

        actual_net_local = (
            o.get("actual_monthly_net_cny")
            or o.get("actual_monthly_net_usd", 0)
            or o.get("actual_monthly_net_sgd", 0)
        )
        actual_net_cny = actual_net_local * rate
        if actual_net_cny > 0 and cal_monthly_cny > 0:
            abs_err_net.append(abs(cal_monthly_cny - actual_net_cny) / actual_net_cny * 100)

        rows.append({
            "id": o.get("id"),
            "region": o["location"],
            "role": o["candidate_role"],
            "currency": o["currency"],
            "calculator_total_local": total.total_comp,
            "calculator_monthly_net_local": total.monthly_net,
            "calculator_total_cny_equiv": cal_total_cny,
            "hr_expected_total_cny": expected_total_cny,
            "hr_actual_monthly_net_local": actual_net_local,
        })
    return {
        "task": "T1802 - offer calibration vs real HR data",
        "n": len(rows),
        "mae_total_pct": round(sum(abs_err_total) / len(abs_err_total), 2) if abs_err_total else None,
        "mae_monthly_net_pct": round(sum(abs_err_net) / len(abs_err_net), 2) if abs_err_net else None,
        "per_offer": rows,
    }
