...
class LLMCache:
    ...
    def get(self, key: str) -> Optional[Any]:
        ...
        with self._stats_lock:
            if value is not None:
                ...
                self._misses += 1
        # T1807: 同步上报到 cache_metrics (跨 namespace 仪表)
        try:
            from .cache_metrics import record_hit, record_miss
            if value is not None:
                record_hit("llm_cache")
            else:
                record_miss("llm_cache")
        except Exception:  # noqa: BLE001
            pass
        return value
    ...
    def set(self, key: str, value: Any, ttl_seconds: Optional[int] = None) -> bool:
        ...
        # T1807: write event
        try:
            from .cache_metrics import record_write
            if ok or mem_ok:
                record_write("llm_cache")
        except Exception:  # noqa: BLE001
            pass
        return ok or mem_ok
