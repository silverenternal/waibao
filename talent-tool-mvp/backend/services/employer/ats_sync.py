...
# ---------------------------------------------------------------- real provider wiring (T1806)
from dataclasses import dataclass as _dc  # noqa: E402


@_dc(slots=True)
class IntegrationSyncResult:
    """单 integration 一次 sync (candidates + jobs) 的合并结果."""

    integration_id: str
    provider: str
    candidates: SyncRunResult
    jobs: SyncRunResult
...
def build_real_provider(
    *,
    ...
    """构造真实 ATS Provider (T1806) — Greenhouse / Lever.

    Args:
        provider_name: "greenhouse" / "lever".
        api_key:       Basic Auth api_key.
        base_url:      可选覆盖 (sandbox / 自部署).
        oauth2_* :     T1806 多租户 OAuth2 (Bullhorn/Workable 备用).
    """
    ...
    oauth2_manager = oauth2_manager  # may be None
    common: dict[str, Any] = {"timeout": timeout}
    if oauth2_token_url and oauth2_tenant:
        if oauth2_manager is None:
            oauth2_manager = OAuth2TokenManager()
        common.update(
            {
                "oauth2_manager": oauth2_manager,
                "oauth2_tenant": oauth2_tenant,
    ...
    if provider_name == "greenhouse":
        return GreenhouseProvider(api_key=api_key, base_url=base_url or "https://harvest.greenhouse.io/v1", **common)
    if provider_name == "lever":
        return LeverProvider(api_key=api_key, base_url=base_url or "https://api.lever.co/v1", **common)
    # 退化到 registry (mock / 其它)
    return make_provider(provider_name, api_key=api_key, base_url=base_url)
...
async def sync_integration(
    ...
    *,
    ...
    provider: ATSProvider,
    since: datetime | None = None,
    triggered_by: str = "real_sync",
) -> IntegrationSyncResult:
    """一次同步: pull candidates + pull jobs. 写各自的 sync_log.

    生产用法:
        engine = ATSSyncEngine(candidates=store, jobs=store, sync_log=store, conflicts=store)
        provider = build_real_provider(provider_name="greenhouse", api_key=..., tenant_id="acme")
