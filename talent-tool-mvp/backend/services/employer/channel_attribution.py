...
class ChannelAttributionService:
    ...
    def _aggregate(
        ...
        for cid, evts in by_candidate.items():
            ...
            # T1803 — multi-touch: 把 metadata.primary_source / secondary_source
            # 也纳入"渠道集合",这样二次触达(re_touch=True)会被算到多渠道。
            for e in evts:
                md = e.metadata or {}
                if md.get("primary_source"):
                    sources.append(str(md["primary_source"]))
                if md.get("secondary_source"):
                    sources.append(str(md["secondary_source"]))
