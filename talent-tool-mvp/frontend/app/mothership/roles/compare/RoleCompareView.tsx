...
export function RoleCompareView() {
  ...
  async function runCompare(idList: string[]) {
    ...
    try {
      const res = await fetchAPI<CompareResponse>(
        `/api/mothership/roles/compare?ids=${encodeURIComponent(idList.join(","))}`
      );
      setData(res);
    } catch (e) {
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3 md:flex-row md:items-end">
            ...
            {data && (
              <SaveCompareButton
                ...
                payload={data}
                defaultTitle={`岗位对比 (${data.items.length} 项)`}
              />
            )}
          </form>
        </CardContent>
      ...
      {!data && !error && !loading && (
        <div className="text-sm text-muted-foreground py-12 text-center">
          ...
          <code className="px-1.5 py-0.5 rounded bg-muted">
            /mothership/roles/compare?ids=id1,id2
          </code>
        </div>
      )}
    </div>
