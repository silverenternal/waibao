<template>
  <view class="strategy">
    <view v-if="!root" class="strategy__loading">Loading strategy map...</view>
...
const root = ref<{ children: StrategyNode[] } | null>(null);

const perspectives: { key: Perspective; label: string; color: string }[] = [
  { key: 'financial', label: 'Financial', color: '#1772F6' },
  { key: 'customer', label: 'Customer', color: '#00B42A' },
  { key: 'process', label: 'Process', color: '#FF7D00' },
  { key: 'learning', label: 'Learning', color: '#9B59B6' },
];

function nodesFor(key: Perspective): StrategyNode[] {
  return (root.value?.children || []).filter((n) => n.perspective === key);
}

function onNode(n: StrategyNode): void {
...
onMounted(() => {
  root.value = {
    children: [
      ...
      { id: 'p2', title: 'Bot resolution 80%', perspective: 'process', progress: 47 },
      { id: 'l1', title: 'Partner training', perspective: 'learning', progress: 33 },
      { id: 'l2', title: 'AI mastery L4', perspective: 'learning', progress: 60 },
    ],
  };
});
</script>
...
.strategy { padding: 24rpx;
  &__loading { padding: 96rpx 0; text-align: center; color: #8A8E99; }
  &__canvas { position: relative; }
  &__lane { position: absolute; left: 0; right: 0; height: 240rpx; }
  &__lane-head { padding: 16rpx 24rpx; border-radius: 12rpx 12rpx 0 0; color: #fff; font-weight: 600; font-size: 26rpx; }
  &__lane-scroll { background: #fff; border-radius: 0 0 16rpx 16rpx; padding: 16rpx; white-space: nowrap; box-shadow: 0 2rpx 8rpx rgba(31,35,41,0.04); }
